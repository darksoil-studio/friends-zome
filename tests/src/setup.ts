...
import { dhtSync, Player, Scenario } from '@holochain/tryorama';
import { EntryRecord } from '@tnesh-stack/utils';
import { dirname } from 'path';
import { fileURLToPath } from 'url';
import { FriendsClient } from '../../ui/src/friends-client.js';
import { FriendsStore } from '../../ui/src/friends-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/friends_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new FriendsStore(
  	new FriendsClient(player.appWs as any, 'friends_test'),
  );
  return {
  	store,
  	player,
